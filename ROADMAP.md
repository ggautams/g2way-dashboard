...
- [x] Saved views, date-range picker, CSV export (ticked when all three below are)
...
- feat(M6): **CSV export** (6b; ADR-0013 §8 added).
  ...
  - Next: 6c, saved views.
- feat(M6): **saved views** (6c; ADR-0016, ADR-0005 amended).
  That completes M6's "saved views, date-range picker, CSV export" box.
  - `saved_views` (migration `0009_saved_views`, both dialects, generated by
    `npm run db:generate -- --name saved_views`): org, environment, owner id
    and email, name, `shared`, `query`, timestamps. Unique per org,
    environment, owner and name.
  - The stored `query` is canonical: `canonicalViewQuery`
    (`src/lib/analytics/saved-views.ts`, pure) re-parses what the form sends
    and writes it back with `drillParams`. So it carries `source` and either
    `range=<id>` (relative) or `from`/`to` (absolute). Key parameters are
    dropped without `keys:read`. Opening a view is just its link; the page
    re-checks everything.
  - Who: personal views need only `gateway:read`, and only the owner lists or
    deletes them (admins too cannot). Sharing, and deleting any shared view,
    needs the new `analytics:share` permission (editor and up). At most 50
    views per person and environment, counted in the write transaction
    (SQLite `BEGIN IMMEDIATE`, Postgres advisory lock per owner).
  - Audit: `analytics.view.create`/`.delete` in the same transaction as the
    write. Refusals are recorded as `denied`, best effort (sharing without
    the permission; deleting someone else's view, which the user is told was
    not found).
  - UI: a "Saved views" `<details>` under the selection bar lists shared,
    then yours, each with its range (marked "fixed window" when absolute),
    source and selection. The current view is marked. The save form and
    delete buttons are small client components taking the server actions as
    props (`saved-view-actions.ts`).
  - Tests: the data layer on SQLite and PGlite (listing, clash, cap, delete
    rules, org isolation, audit rows), the service (permissions, denied
    audits) and the pure module.
  - Surprise: none upstream. Not browser-tested; the visual-pass box lists
    saved views. Follow-up in M12 (what ADR-0016 left out).
  - For "tie ranges to retention": a saved relative view (`range=24h`) opens
    through the same fixed-range code, so it will pick up whatever that task
